...
pub async fn run(
    ...
    match source.as_str() {
        ...
        "archive-org" => {
            ...
            let mut cfg = arachne_tools::adapters::archive_org::ArchiveOrgConfig::new(contact_addr);
            cfg.max_items = limit;
            let (admitted, existing) =
                arachne_tools::adapters::archive_org::harvest(&cfg, repo, nats).await?;
            info!(
                source = "archive-org",
                admitted, existing, "harvest complete"
            );
            println!(
        ...
        "fma" | "fma-large" | "fma-medium" | "fma-small" => {
            ...
            info!(
                source = "fma",
                subset, admitted, existing, "harvest complete"
            );
            println!(
        ...
        other => {
            anyhow::bail!(
                "unknown source '{other}'; available: jamendo, archive-org, fma-small, fma-medium, fma-large"
            );
        }
    }
