...
impl CrawlerMetrics {
    ...
    pub fn new() -> Self {
        ...
        let pages_crawled = IntCounter::new("arachne_pages_crawled_total", "Total pages successfully crawled").unwrap();
        let pages_failed = IntCounter::new("arachne_pages_failed_total", "Total pages failed to crawl").unwrap();
        let urls_discovered = IntCounter::new("arachne_urls_discovered_total", "Total URLs discovered").unwrap();
        let urls_deduped = IntCounter::new("arachne_urls_deduped_total", "Total URLs skipped due to dedup").unwrap();
        let urls_robots_blocked = IntCounter::new("arachne_urls_robots_blocked_total", "Total URLs blocked by robots.txt").unwrap();
        let bytes_downloaded = IntCounter::new("arachne_bytes_downloaded_total", "Total bytes downloaded").unwrap();
        ...
        let active_tasks = IntGauge::new("arachne_active_tasks", "Currently active crawl tasks").unwrap();
        let frontier_size = IntGauge::new("arachne_frontier_size", "Estimated size of URL frontier").unwrap();
        let jobs_running = IntGauge::new("arachne_jobs_running", "Number of currently running jobs").unwrap();

        registry.register(Box::new(pages_crawled.clone())).unwrap();
        registry.register(Box::new(pages_failed.clone())).unwrap();
        registry.register(Box::new(urls_discovered.clone())).unwrap();
        registry.register(Box::new(urls_deduped.clone())).unwrap();
        registry.register(Box::new(urls_robots_blocked.clone())).unwrap();
        registry.register(Box::new(bytes_downloaded.clone())).unwrap();
        registry.register(Box::new(crawl_duration_ms.clone())).unwrap();
        registry.register(Box::new(page_size_bytes.clone())).unwrap();
